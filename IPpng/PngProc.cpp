		   ...
		   )
{
	...
	*/
	...
	int pixel_depth; // always gray 
     qDebug()<<"read image's bit_depth: "<<bit_depth;
    // TODO: need try!!
	switch (color_type)
	{
	...
	*/
	...
	{	
		...
		{
            png_read_rows(png_ptr, &pOut, (png_bytepp)NULL, 1);
			pOut += nRowSize;
		}
	}
	...
	{
        qDebug()<<"NPngProc:readPngFile: Error - %s", e.m_szErrorMessage;
		...
		*/
		...
		else
		{
			...
			{	
				...
				for (unsigned int y = 0; y < height; y++)
				{
                    png_read_rows(png_ptr, &pTempBuf, (png_bytepp)NULL, 1);
